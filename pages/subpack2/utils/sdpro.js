...
function getGridNum(gxy, i, j) {
  let x = Math.ceil(j/gxy.gx);
  let y = Math.ceil(i/gxy.gy);
  let num = (x-1)*gxy.gx + (y-1);
  return num;
}
/**判断结果是否正确
 * type 宫格类型 4、6、9
 * arr 结果数组
 */
function isAllRight(type, arr) {
  let row = []; // 存储每一行的结果集合的数组
  let col = []; // 存储每一列的结果集合的数组
  let grid = []; // 存储每一大格子的结果集合的数组
  let gxy = getgxgy(type); // 每个大格子行列数
  ...
  for (var i = 1; i <= type; i++) {
    for (var j = 1; j <= type; j++) {
      ...
      row[j].add(arr[parseInt(i + '' + j)]);
      col[i].add(arr[parseInt(i + '' + j)]);
      let num = getGridNum(gxy, i, j)+1;
      grid[num].add(arr[parseInt(i + '' + j)]);
    }
  }
  // 判断结果数组中是否满足要求
  for (var i = 1; i <= type; i++) {
    if (row[i].size !== type || col[i].size !== type || grid[i].size !== type) {
      return false;
    }
  }
  return true;
