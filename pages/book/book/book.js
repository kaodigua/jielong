...
Page({
    ...
    onLoad: function (params) {
        ...
        api.book.fetchBook(params.id)
            .then(response => {
                response.cover = formatImageUrl(response.cover,config.cfg.bookApiUrl);
                ...
                const value = wx.getStorageSync("book:"+response._id);
    ...
    addToBookshelf: function () {
        wx.setStorageSync("book:"+this.data.book._id, this.data.book);
    ...
    startRead: function () {
        if (!wx.getStorageSync("book:"+this.data.book._id)) {
            this.addToBookshelf();
        }
        wx.navigateTo({
